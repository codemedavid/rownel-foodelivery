  ...
  const fetchMerchants = async () => {
    try {
      ...
      const formattedMerchants: Merchant[] = (data || []).map((merchant) => ({
        ...
        formattedAddress: merchant.formatted_address,
        osmPlaceId: merchant.osm_place_id,
        ...
        baseDeliveryFee: merchant.base_delivery_fee,
        deliveryFeePerKm: merchant.delivery_fee_per_km,
  ...
  const getMerchantById = async (id: string): Promise<Merchant | null> => {
    try {
      ...
      return {
        ...
        deliveryFeePerKm: data.delivery_fee_per_km,
